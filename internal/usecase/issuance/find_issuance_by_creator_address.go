...
type FindIssuancesByCreatorAddressUseCase struct {
	UserRepository     repository.UserRepository
	IssuanceRepository repository.IssuanceRepository
}

func NewFindIssuancesByCreatorAddressUseCase(userRepo repository.UserRepository, issuanceRepo repository.IssuanceRepository) *FindIssuancesByCreatorAddressUseCase {
	return &FindIssuancesByCreatorAddressUseCase{
		UserRepository:     userRepo,
		IssuanceRepository: issuanceRepo,
	}
}

func (f *FindIssuancesByCreatorAddressUseCase) Execute(input *FindIssuancesByCreatorAddressInputDTO) (*FindIssuancesByCreatorAddressOutputDTO, error) {
	res, err := f.IssuanceRepository.FindIssuancesByCreatorAddress(input.CreatorAddress)
	if err != nil {
		return nil, err
	}
	output := make(FindIssuancesByCreatorAddressOutputDTO, len(res))
