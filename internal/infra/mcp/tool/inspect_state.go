...
// 1. list all open auctions;
// 2. list all orders;
// 3. list auction by creator;
...
func (t *InspectStateTool) ListAllAuctions(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if resp.StatusCode != http.StatusOK {
		...
		return mcp.NewToolResultError(fmt.Sprintf("failed to get auctions: status %d, body: %s", resp.StatusCode, string(body))), nil
	}
...
func (t *InspectStateTool) ListAuctionByCreator(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if resp.StatusCode != http.StatusOK {
		body, _ := io.ReadAll(resp.Body)
		return mcp.NewToolResultError(fmt.Sprintf("failed to get auctions by creator: status %d, body: %s", resp.StatusCode, string(body))), nil
	}
