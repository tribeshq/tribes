...
for i in "${!INVESTOR_ADDRESSES[@]}"; do
    ...
    sendInput '{"path":"create_user","payload":{"address":"'"${INVESTOR_ADDRESSES[$i]}"'","role":"'"$role"'"}}'
    sleep 1
done
...
for i in "${!INVESTOR_ADDRESSES[@]}"; do
    orderPayload='{"path":"create_order","payload":{"crowdfunding_id":1,"interest_rate":"'"${INTEREST_RATES[$i]}"'"}}'
